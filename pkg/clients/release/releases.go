...
func (r *ReleaseController) CreateReleaseServiceConfig(name, namespace string, debug bool, defaultTimeouts *pipeline.TimeoutFields, emptyDirOverrides ...releaseApi.EmptyDirOverride) (*releaseApi.ReleaseServiceConfig, error) {
...
func (r *ReleaseController) PatchReleaseServiceConfig(oldReleaseServiceConfig *releaseApi.ReleaseServiceConfig, emptyDirOverride *releaseApi.EmptyDirOverride) error {
	patch := client.MergeFrom(oldReleaseServiceConfig)
	newReleaseServiceConfig := oldReleaseServiceConfig.DeepCopy()
	newReleaseServiceConfig.Spec.EmptyDirOverrides = append(newReleaseServiceConfig.Spec.EmptyDirOverrides, *emptyDirOverride)
...
func (i *IntegrationController) GetReleaseServiceConfig(namespace string) (*releaseApi.ReleaseServiceConfig, error) {
	rscList := &releaseApi.ReleaseServiceConfigList{}
	opts := []client.ListOption{
		client.InNamespace(namespace),
	}
	err := i.KubeRest().List(context.Background(), rscList, opts...)
