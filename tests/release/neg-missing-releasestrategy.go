...
var _ = framework.ReleaseSuiteDescribe("[HACBS-2360] Release CR fails when missing ReleaseStrategy.", Label("release", "release-neg", "negMissingReleaseStrategy", "HACBS"), func() {
	defer GinkgoRecover()
	...
	BeforeAll(func() {
		...
		fw, err = framework.NewFramework("release-neg-rs-dev")
		Expect(err).NotTo(HaveOccurred())
		devNamespace = fw.UserNamespace

		// Create the managed namespace
		managedNamespace = "release-neg-rs-managed"
